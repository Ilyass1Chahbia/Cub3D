...
/*   Updated: 2024/12/23 19:08:56 by kid-ouis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_button(t_stc *stc, int ms_x, int ms_y)
{
	mlx_get_mouse_pos(stc->mlx_pt, &ms_x, &ms_y);
	if (ms_x > stc->mouse_x)
		return (7);
	else if (ms_x < stc->mouse_x)
		return (8);
	...
	if (mlx_is_mouse_down(stc->mlx_pt, MLX_MOUSE_BUTTON_LEFT))
		return (stc->fire = 1, 7);
	return (0);
}
