...
/*   Updated: 2024/12/24 13:13:21 by kid-ouis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	vr_ray(t_stc *stc, t_line *line)
{
	...
	vs.inter_y = stc->player.y + (vs.inter_x - stc->player.x)
		* tan(line->ray_ang);
	line->v_step_x = SZ_TILE;
	line->v_step_y = SZ_TILE * tan(line->ray_ang);
	if (line->ray_left)
		line->v_step_x *= -1;
	if (line->ray_up && line->v_step_y > 0)
		line->v_step_y *= -1;
	if (line->ray_down && line->v_step_y < 0)
		line->v_step_y *= -1;
	vs.vz_x = vs.inter_x;
	vs.vz_y = vs.inter_y;
...
void	hz_ray(t_stc *stc, t_line *line)
{
	...
	line->h_step_y = SZ_TILE;
	if (line->ray_up)
		line->h_step_y *= -1;
	line->h_step_x = SZ_TILE / tan(line->ray_ang);
	if (line->ray_left && line->h_step_x > 0)
		line->h_step_x *= -1;
	if (line->ray_right && line->h_step_x < 0)
		line->h_step_x *= -1;
	vs.hz_x = vs.inter_x;
	vs.hz_y = vs.inter_y;
